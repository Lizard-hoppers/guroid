...
# Состояния ончейн-проверки (ТЗ «Верификация транзакций», раздел 2,
# 06.09.2026). Раньше был один булев tx_verified, который поднимался по
# факту существования транзакции, без сверки суммы.
TX_STATE_NONE = "none"          # не проверяли: нет хеша / нечего сверить / API молчит
TX_STATE_MISMATCH = "mismatch"  # не найдена, провалена или сумма не сошлась
TX_STATE_VERIFIED = "verified"  # успешна И сумма сошлась
TX_STATES = (TX_STATE_NONE, TX_STATE_MISMATCH, TX_STATE_VERIFIED)

# Допуск при сверке (раздел 3): ±2% или ±$5 — что больше. Комиссию удерживают
# биржи и обменники, плюс люди округляют суммы при вводе; сверка «до цента»
# дала бы поток ложных расхождений на честных сделках.
# Давность транзакции (ТЗ «Hash_Uniqueness», раздел 5). Старше — сделка
# создаётся, но ончейн-подтверждение не применяется.
TX_MAX_AGE_DAYS = 365

# 11.09.2026 — потолок для отсева ВРУЧНУЮ выданных подписок (владелец
# правит subscription_expires_at напрямую в БД, обходя приложение,
# срок получается ~100 лет вперёд). Ни один настоящий тариф не продаёт
# больше года за раз, 3 года — большой запас даже на несколько подряд
# продлений, но отсекает любую ручную правку на десятилетия.
SUBSCRIPTION_REALISTIC_MAX_YEARS = 3

TX_AMOUNT_TOLERANCE_PCT = 0.02
TX_AMOUNT_TOLERANCE_ABS = 5.0
...
# Анти-фрод (ТЗ п.4, исходный план 04.08): ограничение числа новых заявок
# на ПАРУ контрагентов за скользящие сутки, и подтверждения между свежими
# (<14 дней в комьюнити) аккаунтами не учитываются в рейтинге (видны, но
# помечены).
#
# 08.09.2026, просьба владельца: было 1 в сутки, стало 3. Важно, что это
# именно СЧЁТЧИК за окно, а не пауза между заявками: с паузой в 8 часов
# три сделки подряд с одним человеком всё равно не завести, а нужно
# наоборот — рекрутер закрыл с компанией три найма и заводит их сразу.
RATE_LIMIT_HOURS = 24
RATE_LIMIT_REQUESTS_PER_PAIR = 3
...
# linkedin (05.09.2026) — раньше приходил только из анкеты бота и в
# Mini App не правился, хотя соседний website правился. Теперь это
# такое же extra-поле guro_users с фолбэком на анкету (см.
# _profile_summary в guro_id_api.py), как сделано для looking_for.
EXTRA_PROFILE_FIELDS = ("cv_text", "website", "offering", "looking_for", "linkedin")

# Поля САМОЙ анкеты, которые разрешено править в Mini App (09.09.2026,
# просьба владельца). Раньше анкета заполнялась только ботом.
#
# vertical и grade проверяются по справочнику (constants.VERTICALS/GRADES):
# по ним работает поиск — browse сравнивает вертикаль с каноническим
# значением, грейд ищется подстрокой. Свободный текст здесь означал бы
# человека, которого не находит ни один фильтр.
#
# username не правим (приходит из Telegram), country тоже: рядом лежит
# country_iso2 из отдельной нормализации, на нём держатся сегменты
# рассылки по странам, и правка половины пары их тихо сломает.
ANKETA_EDITABLE_FIELDS = ("name", "company", "profession")
ANKETA_CHOICE_FIELDS = ("vertical", "grade")
