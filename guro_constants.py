...
# (requester) в поиске, тумблеры приватности ЦЕЛИ игнорируются — видно
# всё, даже если владелец профиля не открыл НИ ОДНОГО show_*-тумблера.
# НЕ влияет на «рейтинг сгорает без подписки» (_apply_subscription_gate) —
# это ДРУГАЯ, независимая ось (зависит от подписки ЦЕЛИ, не запрашивающего),
# трогать её не просили. НЕ влияет на приватность кабинета рекрутера
# (_apply_recruiter_privacy) — не запрашивалось, отдельная фича при
# необходимости. Список короткий и захардкожен намеренно — это ручная
# административная привилегия, не публичная функция продукта.
