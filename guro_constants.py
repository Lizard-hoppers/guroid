...
# владелец профиля решает в Mini App, что из анкеты видно чужим в поиске.
# По умолчанию НИЧЕГО не видно (все флаги = 0/False) — участник сам
# включает то, что хочет показать. Партнёрства (с кем сотрудничал) и
# username показываются ВСЕГДА, независимо от этих тумблеров — это ядро
# смысла GURO ID (подтверждение репутации через сотрудничество) и
# необходимый минимум для идентификации в поиске. show_tenure открывает
# ОБА поля стажа разом (joined_community_at и days_in_community — это
# одно и то же для пользователя).
PRIVACY_FIELDS = ("show_name", "show_company", "show_vertical", "show_tenure", "show_reputation")
